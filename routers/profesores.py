...
from configuration import get_session,supabase  # Importa la función get_session para obtener una sesión de base de datos y 
...
def update_profesor(profesor_id:UUID, profesor_data:ProfesorUpdate, usuario_data:UsuarioUpdate, session:Session=Depends(get_session)) -> dict:
    profesor = session.get(Profesor, profesor_id)
    if not profesor:
        raise HTTPException(404, "Profesor no encontrado")
